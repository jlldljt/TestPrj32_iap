...
//	SPI_Flash_Read((CDV_INT08U *)&RW_SRP_NUM_DEBUG,SCRIP_NUM_DEBUG,2);
	...
	memset(&g_modbusCoil , 0xFF,sizeof(g_modbusCoil));
...
RET_STATUS ResParse(CDV_INT08U* rxBuf, CDV_INT08U rxLen , CMD_ARG *arg) {
	...
	switch(rxBuf[0]){
		case 0x09:/*显示命令*/
			//ret = DisplayCmd(rxBuf + 1, rxLen - 1, debug, arg->uart);
			break;
		case 0x00:/*主轴命令*/
		...
		case 0x02:/*O*/
			...
			break;
		case 0x03:/*定时器*/
			ret = TimeCmd(rxBuf + 1, rxLen - 1, arg->uart);
			break;
		case 0x04:/*消息*/
			ret = MsgCmd(rxBuf + 1, rxLen - 1);
			break;
		case 0x05:/*变量*/
			ret = ValCmd(rxBuf + 1, rxLen - 1, arg);
			break;
		case 0x06:/*料盘*/
			//ret = TrayScript(rxBuf + 1,SCRIP_TRAY, debug, uartNo);
			break;
		case 0x07:/*电缸*//*串口资源*/
			//ret = DG_RUN(rxBuf + 1,rxLen - 1, uartNo);
//			delay_ms(500);
			break;
		case 0x08:/*逻辑资源*/
//			if(2 == debug)
//				ModbusRequest(*(rxBuf-1), 00, uartNo);
			ret = LogicScript(rxBuf+1,arg);
			break;
		case 0x0A:
//			ret = PromptCmd(rxBuf + 1,rxLen - 1, uartNo);
			break;
		case 0x0B:
		...
		case 0x0C:
			...
			break;
//		case 0x10:/*直线运动*/
//			ret =Line_Interpolation(rxBuf + 1,rxLen - 1, debug);
//			break;	
		case 0x0D:/*计时器*/
			ret = ClockCmd(rxBuf + 1, rxLen - 1, arg->uart);
			break;
		case 0x0E:/*工人*/
	...
	if(OPT_FAILURE == ret) {
		OSTimeDlyHMSM(0,0,0,1,OS_OPT_TIME_HMSM_STRICT,&err);   //延时 1ms
		//OSSched();
	}
	g_nowDoRes = 0;
	return ret;
