...
u8 gmoto_flag;
u8 gmoto_rzflag=0;//归零标记，有归零命令就置1
u8 gmoto_suspend_flag=0;//电机暂停标记，有暂停命令就置1
u32 bufmax_speed;
...
u8 Moto_Config(u8 Moto_No,u32 Moto_Pflag,u8* rxBuf,u8 type);
...
u8 Moto_MultSpeen_Config(u8 Moto_No);
...
//	 }
	 if(Moto_Pflag & 0x4000)//多段速参数设置
	 {
	   ...
	   m_speed_num = moto_par.mult_speed_num[Moto_No];	 
		...
		for(j=0;j<10;j++)
		 {
			moto_par.mult_a_d_speed[Moto_No][j]=UartToU32(&rxBuf[i],4,BIGENDIAN);//(rxBuf[i]<<24)+(rxBuf[i+1]<<16)+(rxBuf[i+2]<<8)+rxBuf[i+3];
			ParRangeCtrl(&moto_par.mult_a_d_speed[Moto_No][j],moto_par.max_acc[Moto_No],moto_par.min_acc[Moto_No]); 
			moto_par.mult_speed[Moto_No][j]=UartToU32(&rxBuf[i+4],4,BIGENDIAN);//(rxBuf[i+4]<<24)+(rxBuf[i+5]<<16)+(rxBuf[i+6]<<8)+rxBuf[i+7];
		   ParRangeCtrl(&moto_par.mult_speed[Moto_No][j],moto_par.max_speed[Moto_No],moto_par.min_speed[Moto_No]); 
			moto_par.mult_step[Moto_No][j]=UartToU32(&rxBuf[i+8],4,BIGENDIAN);//(rxBuf[i+8]<<24)+(rxBuf[i+9]<<16)+(rxBuf[i+10]<<8)+rxBuf[i+11];
...
//	 }
	 ...
	 if(Moto_Pflag & 0x00010000)//目标运行速度
	 {
		   if(type == 0xFD)i++;
			...
			if(!(Moto_Pflag & 0x0010))
			{
				...
				buf_data=Moto_Dec_Con(dec,moto_par.target_speed[Moto_No],moto_par.min_speed[Moto_No]);			
				if((Moto_No&0x01)==0)
					buf[0]=0x00;
				else if((Moto_No&0x01)==1)
					buf[0]=0x02; 
	 ...
	 if(Moto_Pflag & 0x0010)//减速度
	 {
		  ...
		  moto_par.dec[Moto_No] = UartToU32(rxBuf+i,4,LITTLEEND);
			...
			buf_data=Moto_Dec_Con(moto_par.dec[Moto_No],moto_par.target_speed[Moto_No],moto_par.min_speed[Moto_No]);			
			if((Moto_No&0x01)==0)
				buf[0]=0x00;
			else if((Moto_No&0x01)==1)
				buf[0]=0x02; 
...
//		Send_parameter(Moto_No>>1,buf,FPGA_COMMAND_LEN);	
		...
		Send_parameter(Moto_No>>1,buf,FPGA_COMMAND_LEN);		
		if(state==0||gmoto_flag==1)//停止点动或者上一次暂停之后没继续，获取当前位置，更新当前位置至目标位置  
		{			
			if(Moto_No%2)
...
//		  buf[2] = len>>16;
			...
			//编码器高位
			//len=len*1024/((moto_par.div_num[Moto_No][0]<<8)+moto_par.div_num[Moto_No][1]);
			//len = (CDV_INT32U)((CDV_INT64U)len*(CDV_INT64U)UartToU32(moto_par.encoder_pre[Moto_No],4,LITTLEEND)/(CDV_INT64U)UartToU32(moto_par.div_num[Moto_No],2,BIGENDIAN));
			len = (CDV_INT32U)((CDV_INT64U)len*(CDV_INT64U)moto_par.encoder_pre[Moto_No]/(CDV_INT64U)moto_par.div_num[Moto_No]);
			len = len==0?1:len;
			if((Moto_No&0x01)==0)
				buf[0]=0x00;
...
float Moto_Speed_Con(u32 user_data)
{
	 float FPGA_Data;
	#if 1
	FPGA_Data = user_data*FPGA_ACC_DIV;
	 #else
	  if(user_data<20)
		{
			user_data=20;
...
//	{	
//	  b=max_speed-min_speed;//△V 开发层
//	}
//  else if(max_speed<min_speed)
//	{	
//		b=min_speed-max_speed;//△V 开发层
//  } 
//  else 
//  {
//		b=1;
//	}  		
//	t=b*1000/user_data;//ms
//	
//	c=10000000/min_speed-10000000/max_speed;//△V FPGA
//	
//	FPGA_Data=c*5/t;
//	#else
//		FPGA_Data = FPGA_W_FRE_DIV2*FPGA_SPEED_CHANGE_CYCLE*user_data/(min_speed*max_speed);
//	#endif
//	//推倒出 FPGA_Data = 50000*user_data/(min_speed*max_speed);
//	if(FPGA_Data>255)FPGA_Data=255;
//	if(FPGA_Data<1)FPGA_Data=1;
//	return (u16)FPGA_Data;
}
...
u32 Get_Dstep(u32 max_speed,u32 min_speed,u32 dec_speed)
{
	...
	dec_data=(float)dec_speed;
		...
		count=(u16)(abs(max_speed-min_speed)/(dec_data*FPGA_SPEED_CHANGE_CYCLE));//速度变化次数
		if(count<1)count = 1;
		for(i=0;i<count;i++)
		{
			b=FPGA_SPEED_CHANGE_CYCLE*a;  
...
u8 MOTO_ACC_DEC(u8 Moto_No,u32 run_step,MOTOR_INFO *moto_pared)
{
	 ...
	 u32 DecStep;
	 u32 AccStep;
//	 max_speed=(moto_par.target_speed[Moto_No][0]<<24)+(moto_par.target_speed[Moto_No][1]<<16)+(moto_par.target_speed[Moto_No][2]<<8)+moto_par.target_speed[Moto_No][3];
//	 min_speed=(moto_par.min_speed[Moto_No][0]<<24)+(moto_par.min_speed[Moto_No][1]<<16)+(moto_par.min_speed[Moto_No][2]<<8)+moto_par.min_speed[Moto_No][3];
//   acc=(moto_par.acc[Moto_No][0]<<24)+(moto_par.acc[Moto_No][1]<<16)+(moto_par.acc[Moto_No][2]<<8)+moto_par.acc[Moto_No][3];
//   dec=(moto_par.dec[Moto_No][0]<<24)+(moto_par.dec[Moto_No][1]<<16)+(moto_par.dec[Moto_No][2]<<8)+moto_par.dec[Moto_No][3];
      ...
      DecStep=Get_Dstep(max_speed,min_speed,dec);//得到减速需要的步数
		  if((AccStep+DecStep)<run_step)//如果加速步数+减速步数小于总步数
			{	
				  ...
				  {
						  //最大速度
							buf_data=Moto_Speed_Con(max_speed);			
							if((Moto_No&0x01)==0)
							{	
								buf[0]=0x00;
							}	
			...
			{	
				 ...
				 DecStep=run_step/2; //减速步数=1/2总行程
					//发送加速度
					buf_data=Moto_Acc_Con(dec,bufmax_speed,min_speed);
					if((Moto_No&0x01)==0)
						buf[0]=0x00;
					else if((Moto_No&0x01)==1)
						buf[0]=0x02;
					...
					Send_parameter(Moto_No>>1,buf,FPGA_COMMAND_LEN);
					//发送减速度,减速度=加速度
					buf_data=Moto_Dec_Con(dec,bufmax_speed,min_speed);
					if((Moto_No&0x01)==0)
						buf[0]=0x00;
					else if((Moto_No&0x01)==1)
						buf[0]=0x02;
   ...
   else//如果加速步数大于总行程
   {
      ...
      DecStep=run_step/2;//减速步数=1/2总行程
			//发送加速度
			buf_data=Moto_Acc_Con(acc,bufmax_speed,min_speed);
			if((Moto_No&0x01)==0)
				buf[0]=0x00;
			else if((Moto_No&0x01)==1)
				buf[0]=0x02;
			...
			//发送减速度
			buf_data=Moto_Dec_Con(acc,bufmax_speed,min_speed);
			if((Moto_No&0x01)==0)
				buf[0]=0x00;
			else if((Moto_No&0x01)==1)
				buf[0]=0x02;
		...
		switch(opt) {
			...
			case 0x05://暂停/继续
								if((rxBuf[3] & 0x01)==1)//暂停
								{
									...
									gmoto_flag=1;
									gmoto_suspend_flag=1;
								}	
								else if(gmoto_rzflag==1)//如果归零过程中暂停就继续归零
								{
									if(gmoto_suspend_flag==1) 
									{	
			...
			case 0x07://参数设置 
							 switch(type) 
							 {
								...
								case 0x2F:
//										 if(pflag>0xFFFF)
											Moto_Config(Moto_No,pflag,rxBuf+7,type);
//										 else
...
//										 break;
			...
			case 0x09://点动
							if((rxBuf[3]&0x01)==1)//点动向前
							 {	 									
									ret=Motor_Jog_Run(Moto_No ,1,rxBuf[3]>>1);
...
//					if(gmoto_flag==1)//如果上一次暂停之后没继续，就要从新读取FPGA当前的坐标
//					{
//						gmoto_flag=0;
...
//					{
//						 case 0xFF:
//											num = (CDV_INT32S)UartToU32(&rxBuf[3],4,LITTLEEND);
...
//											{	
//												ret=Motor_MultSpeen_Run(Moto_No ,num,0);	
//												rdir=1;
//												gmoto_pos_start[Moto_No]=gmoto_pos[Moto_No];
//												gmoto_pos[Moto_No]=gmoto_pos[Moto_No]-num;
//												gmoto_flag=0;
...
//						buff[1]=0;
//						buff[2]=0;
//						FPGA_Send_parameter(buff,Moto_No);
//					break;
		  default:
							break;
...
//			 buf[2]=Speed_Data;
	 ...
	 {  
			...
			Speed_Data=Moto_Dec_Con(Dec_Speed,Max_Speed,Min_Speed);
//			buf[0]=0x34;
//			buf[1]=Speed_Data>>8;
//			buf[2]=Speed_Data;
			buf[0]=0x04;
...
{
//	motor->acc[Moto_No]
	u32 runtime,acctime,dectime,uniformtime;
	...
	AccStep = Get_Dstep(TargetSpeed,MinSpeed,acc);
		...
		if(moto_par.speed_type[Moto_No]==TRAP_MODE)
		{
			...
			{
				acctime = (TargetSpeed-MinSpeed)*1000/acc;//加速时间
				dectime = (TargetSpeed-MinSpeed)*1000/dec;//减速时间
			...
			else if(AccStep>run_step)//加速步数大于总步数  减速度改成加速度
			{
				...
				acctime = (MaxSpeed-MinSpeed)*1000/acc;//加速时间
				runtime = acctime*2;		
			}
		...
		switch(type) 
		{
			case 0xFF: 
								if(rxLen>10)//需要同时设置其他参数
								{
									...
									Moto_Config(Moto_No,pflag,rxBuf+9,type);								
								}
								else
								{
									num = (CDV_INT32S)UartToU32(&rxBuf[1],4,LITTLEEND);
			...
			case 0xFD://参数混合
								...
								Moto_Config(Moto_No,pflag,rxBuf+10,type);	
								break;
			default:
							break;
		}
...
//							num = VarGet(variate_num);
	...
	if((num<=up_limit)&&(num>=down_limit))
	{	
