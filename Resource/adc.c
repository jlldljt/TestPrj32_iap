...
u16 adc2_value[adc2_value_lens]={0};
u16 adc3_value[adc3_value_lens]={0};
...
{	
	...
	memset(buff_x,0,sizeof(CDV_INT16U)*BUFFX_LEN);		
	memset(buff_v,0,sizeof(CDV_FP32)*BUFFV_LEN);	
...
//						if(ad_num==0)
//						{	
//							R_AD_COEFFICIENT	=	(rxBuf[5+i]<<24)+(rxBuf[6+i]<<16)+(rxBuf[7+i]<<8)+rxBuf[8+i];
//						}
//						else if(ad_num==1)
//						{
//							R_AD_COEFFICIENT	=	((rxBuf[5+i]<<24)+(rxBuf[6+i]<<16)+(rxBuf[7+i]<<8)+rxBuf[8+i])/10000;
//						}			
							if((type==0xFF)||(type==0x2F))
							 {	 
									if(pflag & 0x00000001) //斜率值
									{	
										adc_par.Slope[no]=(rxBuf[13+i]<<24)+(rxBuf[12+i]<<16)+(rxBuf[11+i]<<8)+rxBuf[10+i];
										if(no==2)
									...
									{	
										adc_par.Zero[no]=(rxBuf[13+i]<<24)+(rxBuf[12+i]<<16)+(rxBuf[11+i]<<8)+rxBuf[10+i]; 
										if(no==2)
									...
									{	
										adc_par.Out_Min[no]=(rxBuf[13+i]<<24)+(rxBuf[12+i]<<16)+(rxBuf[11+i]<<8)+rxBuf[10+i];
										i=i+4;
									}	
									if(pflag & 0x00000008)//输出最大值
									{	
										adc_par.Out_Max[no]=(rxBuf[13+i]<<24)+(rxBuf[12+i]<<16)+(rxBuf[11+i]<<8)+rxBuf[10+i];
										i=i+4;
									}
									if(pflag & 0x00000010)//临时斜率值
									{	
										adc_par.Slope2[no]=(rxBuf[13+i]<<24)+(rxBuf[12+i]<<16)+(rxBuf[11+i]<<8)+rxBuf[10+i];
										if(no==2)
									...
									{	
										adc_par.Zero2[no]=(rxBuf[13+i]<<24)+(rxBuf[12+i]<<16)+(rxBuf[11+i]<<8)+rxBuf[10+i];
										if(no==2)
							 ...
							 else if ((type==0xFE)||(type==0x2E)) //按变量设参数
							 {
								  var_num = (rxBuf[13+j]<<24)+(rxBuf[12+j]<<16)+(rxBuf[11+j]<<8)+rxBuf[10+j]; //取变量号
									...
									if(pflag & 0x00000004)//输出最小值
									{	
										adc_par.Out_Min[no]=var_val;
									}	
									if(pflag & 0x00000008)//输出最大值
									{	
										adc_par.Out_Max[no]=var_val;
									}
									if(pflag & 0x00000010)//临时斜率值
									{	
										adc_par.Slope2[no]=var_val;
										if(no==2)
											R_AD_COEFFICIENT_2 = (CDV_FP64)adc_par.Slope2[no]/10000;	
										else if(no==0)
										{
		...
		case 0x02:
			        if(ad_num==1)//获取变频器转速
							{	
								...
								VFD_Speed=Get_VFD_Speed_CH((u16*)(adc3_value2), 100, 2);
...
u8 Read_Adc_Par(u8 *buf,u8 no,u32 pflag)
{
	...
	else if(no==2)
	{
	 ...
	 if(pflag & 0x00000001)//斜率值
	 {
			for(j=0;j<4;j++)
			{
				*(buf+i+j)=adc_par.Slope[no]>>(24-8*j);
			}
			i=i+4;
	 }
	 if(pflag & 0x00000002)//零位值
	 {
			for(j=0;j<4;j++)
			{
				*(buf+i+j)=adc_par.Zero[no]>>(24-8*j);
			}
			i=i+4;
	 }
	 if(pflag & 0x00000004)//输出最小值
	 {
			for(j=0;j<4;j++)
			{
				*(buf+i+j)=adc_par.Out_Min[no]>>(24-8*j);
			}
			i=i+4;
	 }
	 if(pflag & 0x000000008)//输出最大值
	 {
			for(j=0;j<4;j++)
			{
				*(buf+i+j)=adc_par.Out_Max[no]>>(24-8*j);
			}
			i=i+4;
	 }
	 if(pflag & 0x00000010)//临时斜率值
	 {	
		  for(j=0;j<4;j++)
			{
				*(buf+i+j)=adc_par.Slope2[no]>>(24-8*j);
			}
			i=i+4;									
	 }	
	 if(pflag & 0x00000020)//临时零位值
	 {	
		 for(j=0;j<4;j++)
			{
				*(buf+i+j)=adc_par.Zero2[no]>>(24-8*j);
			}
			i=i+4;						
	 }
	return i;//返回长度
